...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private var isPrefernceEmpty = false
    private lateinit var userModel: UserModel

    private lateinit var binding: ActivityMainBinding

    // Di sini kita lihat, untuk memulai activity kita tidak menggunakan startActivity,
    // melainkan menggunakan metode launch dari ActivityResultLauncher hasil dari registerForActivityForResult.
    ...
    private fun checkForm(userModel: UserModel) {
        when {
            userModel.name.toString().isNotEmpty() -> {
                ...
                isPrefernceEmpty = false
            }
            else -> {
                binding.btnSave.text = getString(R.string.save)
                isPrefernceEmpty = true
            }
        }
    }
